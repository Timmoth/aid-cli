...
fn plot_chart(points: &Vec<(f32, f32)>, expression: String) {
    let width = 50;
    let height = 20;
...
pub fn plot(startx: f32, endx: f32, step: f32, expression: String) {
    // Example points
    let points = generate_points(startx, endx, step, &expression);
    // Plot the chart
    plot_chart(&points, expression);
}

fn generate_points(start: f32, end: f32, step: f32, expression: &String) -> Vec<(f32, f32)> {
